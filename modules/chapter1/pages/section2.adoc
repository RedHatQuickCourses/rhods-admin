...
+
 * *Dashboard:* Dashboard provides with a web based user interface to Openshift Data Science.
+
...
+
 * *ModelMeshServing:* ModelMeshServing is a platform that focuses on serving machine learning models efficiently in production. It aims to provide a unified serving layer for various machine learning frameworks and languages. The goal is to offer high-performance, low-latency model serving with features like model versioning, A/B testing, and multi-model serving.
+
...
+
[IMPORTANT]
Make sure that you install all pre-requisities for selected components. For more details see the xref::section1.adoc[General Information about Installation] section.
+
For this demonstration you can accept the default (pre-selected) components selection - Dashboard, Data Science Pipelines, Model Mesh Serving and Workbenches.
+
