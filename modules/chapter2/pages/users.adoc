...
The terms `Namespaces`, OpenShift `Projects`, and `Data Science Projects` are often used synonymy.  While there are slight differences between these different objects, a project is essentially a namespace with some additional features provided by OpenShift.  Similarly, a Data Science Project is just an OpenShift Project that has a specific label, which enables it to be used with the Dashboard.
